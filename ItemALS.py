...
def parse_rating(line, sep=','):
    ...
    Returns: tuple of (random integer, (user_id, item_id, rating))
    """
    fields = line.strip().split(sep)
    user_id = int(fields[0]) # convert user_id to int
    item_id = int(fields[1])  # convert profile_id to int
    rating = float(fields[2])  # convert rated_id to int
    return random.randint(1, 10), (user_id, item_id, rating)
...
def parse_user(line, sep=','):
    ...
    """
    fields = line.strip().split(sep)
    item_id = int(fields[0]) # convert user_id to int
    location = fields[1]
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print "Incorrect number of arguments, correct usage: item_recommender.py [user_id]  [match_location]"
        sys.exit(-1)

    # Configure Spark
    conf = SparkConf().setAppName("Item Recommender") 

    sc = SparkContext(conf=conf)

    matchseeker = int(sys.argv[1])
    location_filter = sys.argv[2]

    # Create ratings RDD of (randint, (user_id, profile_id, rating))
    ratings = sc.textFile("hdfs:///user/maria_dev/nextDoor2/views.csv") \
                 .map(parse_rating)

    # Create users RDD
    users = dict(sc.textFile("hdfs:///user/maria_dev/nextDoor2/location.csv") \
                   .map(parse_user) \
                   .collect())
    ...
    partners = sc.parallelize([u[0] for u in filter(lambda u: u[1] == location_filter, users.items())])
